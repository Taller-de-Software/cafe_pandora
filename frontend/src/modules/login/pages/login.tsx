...
function Login() {
  ...
  const { data: usuariosDisponibles = [], isLoading: cargandoUsuariosQuery } = useQuery({
  ...
  async function handleRegister(e: FormEvent) {
    e.preventDefault()
    // Para registro seguimos usando el formulario original si se desea
    const nombre = (e.target as HTMLFormElement).elements.namedItem('nombre') as HTMLInputElement
    ...
    setLoading(true)
    try {
      await registerUser({ nombre: nombre.value.trim(), pin: pin || undefined })
      navigate('/dashboard')
    } catch (err) {
      showError(err)
    } finally {
          ...
          <div className={styles.formContainer}>
            ...
            {tab === 'login' ? (
              <form className={styles.form} onSubmit={handleLogin}>
                ...
                <button type="submit" className={styles.btn} disabled={loading || !usuarioId || cargandoUsuarios}>
